...
class VectorStore:
    ...
    def __init__(self, config):
        ...
        # Initialize a real dense embedding model. The previous project version
        # left this as None, which caused Qdrant ingestion/retrieval to fail.
        self.embedding_model_name = config.rag.embedding_model_name
        ...
        Path(self.vectorstore_local_path).parent.mkdir(parents=True, exist_ok=True)
        Path(self.docstore_local_path).mkdir(parents=True, exist_ok=True)
        Path(config.rag.parsed_content_dir).mkdir(parents=True, exist_ok=True)
